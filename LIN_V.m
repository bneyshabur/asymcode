function [Wq, V] = LIN_V( X, S, K )
% Given a binary similarity matrix S, the bit length K and feature vectors X, this function returns Wq to be used as linear hash functions for query and V as the codes for database objects such that the hamming distance of binary codes between query and databased objects corresponds to the similarity matrix.
% 
% Input:
...
% V:              A K*N matrix that the binary codes for all objects in the training set as database objects.
  ...
  [Wq, V] = LIN_V_logistic( X, S, K, param, verbose );
