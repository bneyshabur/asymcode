function [Wq, Wd] = LIN_LIN_logsitic( X, S, K, param, verbose )
...
% Wd:         A K*d matrix we use to generate the binary codes for database objects where sign(Wd*X) will be the binary codes for all objects in the training set as database objects.
  ...
  Wd = Wq;                                % Initializes Wd to be equal to Wq (symmetric initialization)
  ...
  D = updateD(X,X,S,Wq,Wd,K,nsamples,subloss);
  ...
  while(1)
    ...
    for i=1:K
      ...
      [wq, wd] = updateW( S, X, Wq(1:end~=ind,:), Wd(1:end~=ind,:), loss, param, Wq(ind,:)', Wd(ind,:)' );
      ...
      new_D = updateD( X, X, S, new_Wq, new_Wd,K, nsamples, subloss);
...
% =============================================================
% =============================================================
function D = updateD( X1, X2, S, Wq, Wd, K, nsamples, subloss )
...
% X2:         A d*N2 matrix that is d-dimensional features for N2 objects
% S:          An N1*N2 similarty matrix of two sets of objects
% Wq:         A K*d matrix used in the hash function for query objects
% Wd:         A K*d matrix used in the hash function for database objects
...
% D:          A cell strcuture that includes the needed statistics of the current binary codes
  ...
  % U and V are K*nsamples binary matrix 
  U = sign(Wq*x1);
  V = sign(Wd*x2);
